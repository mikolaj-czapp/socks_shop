...
from apps.cart.models import ProductCart, Cart
from apps.order.models import Order
from apps.product.models import Brand, Category, Product
from apps.user.models import MyUser


class OrderByUser(TestCase):
    def setUp(self):
        # create two users
        ...
        # Create an order
        test_shipping_address = 'test_address'
        test_ordered_on = datetime(2021, 9, 11, 12, 36, 35, 311600)
        test_order = Order.objects.create(
            user=test_user_1,
            cart=test_cart_1,
            shipping_address=test_shipping_address,
            ordered_on=test_ordered_on,
            status=True
        )
        test_order.save()

    def test_redirect_if_not_logged_in(self):
        response = self.client.get(reverse('orders_by_user'))
    ...
    def test_order(self):
        ...
        self.assertTemplateUsed(response, 'shopping/orders_by_user.html')